...
class CustomPotion extends PluginCommand{
    ...
    public function __construct($name, Core $main){
        ...
        $this->setDescription("Custom Potions!");
        $this->main = $main;
    }

    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool{
        if(!$sender instanceof Player) return false;
        if($sender->hasPermission("pcp.cp") || $sender->isOp()){
            if(isset($args[0])){
        ...
        if(!$sender->hasPermission("pcp.cmd.cp")){
            $sender->sendMessage(TF::BOLD . TF::DARK_GRAY . "(" . TF::AQUA . "!" . TF::DARK_GRAY . ") " . TF::RESET . TF::GRAY . "You don't have permission to use this command.");
        }else{
            $sender->sendMessage(TF::BOLD . TF::DARK_GRAY . "(" . TF::AQUA . "!" . TF::DARK_GRAY . ") " . TF::RESET . TF::GRAY . "Usage: /potion (player) (potion)");
        }
        return true;
    }
}
